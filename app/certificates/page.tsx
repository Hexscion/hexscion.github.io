...
export default function CertificatesPage() {
  ...
  if (!certificates || certificates.length === 0) {
    return (
      <section className="py-28 px-6 max-w-[1200px] mx-auto min-h-[80vh] flex flex-col items-center justify-center">
  ...
  return (
    <section className="py-28 px-6 max-w-[1200px] mx-auto min-h-[80vh] flex flex-col items-center">
      ...
      <div className="relative w-full max-w-[800px] overflow-hidden rounded-lg aspect-3/2 mt-8 group select-none">
        <div 
          className="flex transition-transform duration-500 ease-in-out h-full"
          style={{ transform: `translateX(-${activeIndex * 100}%)` }}
        >
          {certificates.map((cert) => (
            <div key={cert.id} className="min-w-full h-full flex items-center justify-center px-4 sm:px-8">
              <img 
                src={cert.imgUrl} 
                alt={cert.imgName} 
      ...
      {/* Certificate Caption */}
      <div className="text-center max-w-[600px] animate-fade-in mt-4 sm:mt-8">
        ...
        {certificates.map((_, idx) => (
          <button
            ...
            className={`w-3.5 h-3.5 rounded-full transition-all duration-300 cursor-pointer ${
              idx === activeIndex ? "bg-accent scale-110" : "bg-dark/20 hover:bg-dark/40"
            }`}
